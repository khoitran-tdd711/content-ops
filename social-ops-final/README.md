...
**No Python or terminal on your computer? See [DEPLOY.md](DEPLOY.md)** for a browser-only path (GitHub + Render) that needs nothing installed locally. The instructions below assume you *do* have Python and want to run it yourself (locally, or on your own server).
...
This is a small Flask app with a SQLite database (`social_ops.db`) — one file, easy to back up. For team use it needs to run somewhere always-on and reachable by everyone, not just on one person's laptop. See **[DEPLOY.md](DEPLOY.md)** for a no-install, browser-only path (GitHub + Render, ~$7/month). If you'd rather use your own server, run it with a process manager (e.g. `gunicorn app:app`) behind HTTPS, since it handles login passwords and an API key.
