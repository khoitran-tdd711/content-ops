...
python seed.py          # creates your first boss login
python app.py           # runs on http://localhost:5000
```

Then log in, go to **Team** to add your producers, and **Settings** to connect OneUp (optional but recommended).
...
This is a small Flask app with a SQLite database (`social_ops.db`) — one file, easy to back up. For team use, run it on any internal server or a small host (Railway, Render, a company VM) with a process manager (e.g. `gunicorn app:app`) behind HTTPS, since it handles login passwords and an API key.
